...
ESChildControlList::DrawView(_DC* pDC, _Rect* pRectDC, _Rect* pRectClient){
	...
	*/
	...
	_Font*	pFontNormal		= &m_normalItemTextFont;
	_Font*	pFontSelected	= &m_selectedItemTextFont;
	_Font	fontNormalBold, fontSelectedBold;
...
//		bool	bBold	= IsItemBold(nLoop);
		...
		_Font*	pFontOld = pDC->SelectObject(pFont);
		...
		while( nItem < nItemCt ){
			...
			rcCellDC.right		= min(rcCellDC.left + pItem->m_nWidth, rcItemDC.right);
			...
			if( pItem->m_nHAlign == HAlignment::HAlignement_Middle )
				ptLabel.x = max(ptLabel.x, rcCellDC.left + (rcCellDC.Width() - szText.cx)/2);
			else
			if( pItem->m_nHAlign == HAlignment::HAlignement_Right )
				ptLabel.x = max(ptLabel.x, (rcCellDC.right - szText.cx) - 3);
	...
	*/
	...
	while( nLoop < nCt ){
		...
		if( nOffset >= rcHeaderDC.right || nOffset >= pRectDC->right ) 
			break;
		rcItem.right		= min(rcItem.left + pItem->m_nWidth - nWidthDiv, rcHeader.right);
...
//		pFont = (m_nSortCol == nLoop) ? &fontTextBold : &fontText;
		...
		if( pItem->m_nHAlign == HAlignment::HAlignement_Middle )
			ptLabel.x = max(ptLabel.x, rcItemDC.left + (rcItemDC.Width() - szText.cx)/2);
		else
		if( pItem->m_nHAlign == HAlignment::HAlignement_Right )
			ptLabel.x = max(ptLabel.x, (rcItemDC.right - szText.cx) - 3);
...
ESChildControlList::GetItemRect(int nItem, _Rect& rRect){
	...
	if( nOffsetY < rcItem.bottom && nOffsetY >= rcItem.top ){
		...
		rcItem.bottom	= min(rcItem.top + m_nLineHeight + 1,  m_rRect.bottom);
		rcItem.top		= max(m_rRect.top + nHeaderCY, rcItem.top);
...
ESChildControlList::GetCellRect(int nRow, int nCol, _Rect& rcCell){
	...
	if( nOffsetY < rcItem.bottom && nOffsetY >= rcItem.top ){
		rcItem.top			= (nOffsetY - nTopOffset);
		rcItem.bottom		= min(rcItem.top + m_nLineHeight,  rcClient.bottom);
		rcItem.top			= max(rcClient.top + nHeaderCY, rcItem.top);
		...
		while( nLoop < nColCt ){
			if( nLoop == nCol ){
				rcItem.right	= min(rcItem.left + GetColumnWidth(nCol), rcClient.right);
				rcCell			= rcItem;
				return true;
				}
