...
bool
MercuryBaseView::Redraw(_Rect rcRedraw, ESChildControl* pChildOnly /*= NULL*/){
...
*/
	...
	DCDef dc = GetDC();
    _DC::DrawDC(dc, rcClipBox.left, rcClipBox.top, rcClipBox.Width(), rcClipBox.Height(), &memDC, 0, 0, SRCCOPY);
    // Win
...
bool
MercuryBaseView::DrawBackground(_DC *pDC, _Rect* pRectDC, _Rect* pRectClient, _Rect rcControl, int nMode, COLORREF crFill, _Image* pImageBg, 
	...
	switch( nFillMode ){
		case _FillMode::FillMode_Color:{
			bRet = true;
			pDC->FillSolidRect(rcInvalid, crFill);
			break;
			}
		case _FillMode::FillMode_StretchImage:{
		...
		case _FillMode::FillMode_OmitBorderedImage:{
			if( pImageBg && !pImageBg->IsNull()){
				bRet = true;
		...
		case _FillMode::FillMode_RepeatedImage:{
			if( pImageBg && !pImageBg->IsNull() ){
				bRet = true;
		...
		case _FillMode::FillMode_CentredImage:{
			if( pImageBg && !pImageBg->IsNull() ){
				_Rect rImageCenter;
	...
	// Draw sizebox image. {{
	_Image* pImageSizeBox = &m_info.m_sizeBoxImage;
