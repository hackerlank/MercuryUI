...
void
ESChildControlCheckBox::SetTextFont(_Font* pFont){
	if( !pFont ) return;
...
ESChildControlCheckBox::GetCheckedImage(){
	_Image* pCheckedImage = &m_checkedImage;
...
ESChildControlCheckBox::GetUncheckedImage(){
	_Image* pUncheckedImage = &m_uncheckedImage;
...
bool
ESChildControlCheckBox::SetCheckImages(_Image* pImageCheck, _Image* pImageNormal, bool bRedraw){
...
ESChildControlCheckBox::OnPaintClient(_DC *pDC, _Rect* pRectDC, _Rect* pRectClient){
	...
	if( m_sLabelLocal.length() ){
		...
		_Font*	pFontOld = pDC->SelectObject(&textFont);
